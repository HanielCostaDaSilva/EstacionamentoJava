...
public class Estacionamento {
    ...
    /**
     * checa se a posição inserida é acessível a lista, caso seja, retorna {@code True}.
	 * A posição precisa ser um {@code int }>={@value } e menor que {@value placas.lenght} 
     * @return  a boolean.
     */
	...
	public void sair(int vaga) throws Exception, FilerException {
		...
		OperadorArquivoCSV.escreverArquivo(urlHistorico, this.getMomentoAtual(), Integer.toString(vaga), placa, "saida");
		this.gravarDados();
	...
	public void transferir(int vaga1, int vaga2) throws Exception {
		...
		String placaTransferida = this.placas[vaga2] = this.placas[vaga1];
		this.placas[vaga1] = null;

		// Referente ao histórico


		OperadorArquivoCSV.escreverArquivo(this.urlHistorico, this.getMomentoAtual(), Integer.toString(vaga2), placaTransferida, "Transferência" );
		this.gravarDados();
